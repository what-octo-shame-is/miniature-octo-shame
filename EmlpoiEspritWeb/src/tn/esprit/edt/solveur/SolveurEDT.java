...
public abstract class SolveurEDT {
	 ...
	 * @param creneaux
	 */
	public SolveurEDT(Set<Enseignant> enseignants, Set<Module> modules,
			Set<Pool> pools, Set<Prestation> prestations, Set<Groupe> groupes,
	...
	public String[] verifier() {
		...
		// - un enseignant ne peut pas assurer deux prestations dans un même
		// créneau
		...
		for (Creneau creneau : creneaux) {
			...
			for (Prestation prestation : creneau.getPrestations()) {
				...
				listeEns.add(ens);
				// un enseignant doit être disponible pour assurer une
				// prestation
				...
				for (Groupe groupe : prestation.getGroupes()) {
					// un groupe ne peut pas suivre deux prestations dans un
					// même créneau
					if (listeGrps.contains(groupe))
						conflits.add("Le groupe '" + groupe.getLibelle()
								+ "' suit " + "plusieurs prestations le "
								+ creneau + ".");
					listeGrps.add(groupe);
					// un groupe doit être disponible pour suivre une prestation
					if (groupe.estIndispo(creneau))
						conflits.add("Le groupe '" + groupe.getLibelle()
								+ "' est " + "indisponible le " + creneau + ".");
				}
				// mise à jour du nombre de prestations simultanées pour chaque
				// pool concerné
		...
		for (Groupe groupe : groupes) {
			...
			Map<Module, Integer> nbPrestParModule = new HashMap<Module, Integer>(
					modules.size());
